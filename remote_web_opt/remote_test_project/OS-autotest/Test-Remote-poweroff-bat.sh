...
#		fi
                ...
                sh Test-Remote-poweroff-param.sh $DebugerIP $cmdLine > /dev/null 2>&1
                retCode=$?
                ...
                if [ $retCode -eq 0 ]; then
                  ...
                  echo $ServerIP >> ip_poweroff_test_ok.list
                  #ServerStat='active'
                else
                  failed=$(($failed+1))
                  echo -e "\n\033[32m$DebugerIP | failed\033[0m\n"
                  echo $ServerIP >> ip_poweroff_test_error.list
                  #DebugerStat='dead'
                fi
...
rm -f ip_poweroff_test_ok.list
rm -f ip_poweroff_test_error.list
...
echo ""
if [ $poweroffTestTag -eq 0 ]; then
  echo "The IP list of successful remote shutdown is as follows:"
  cat ip_poweroff_test_ok.list
fi
